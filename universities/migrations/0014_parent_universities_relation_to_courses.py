...
class Migration(DataMigration):
    depends_on = (
        ("courses", "0031_auto__add_unique_courseuniversityrelation_university_course"),
    )
    ...
    def forwards(self, orm):
        ...
        obsolete_universities = orm.University.objects.filter(
            parent__isnull=False
        )
        CourseUniversityRelation = orm['courses.CourseUniversityRelation']
        for obsolete_university in obsolete_universities:
            real_university = obsolete_university.parent
            for course in obsolete_university.courses.all():
                try:
                    CourseUniversityRelation.objects.create(
                        university=real_university,
                        course=course,
                        order=10,
                    )
                    logger.info(
                        u'Data Migration: Created relation for '
                        '{} and {}'.format(course.key, obsolete_university.name)
                    )
                except IntegrityError as e:
                    logger.info(
                        u'Data Migration: Did not create relation for '
                        '{}. \n{}'.format(course.key, e)
                    )
    ...
    complete_apps = ['courses', 'universities']
    symmetrical = True
