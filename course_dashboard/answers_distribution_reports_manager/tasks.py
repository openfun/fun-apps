...
def create_list_of_question_ids(organisation, course_number, problem_module_id,
                                problem_module_size):
    question_ids = []
    i = 0

    template = "i4x-{}-{}-problem-{}_".format(organisation, course_number,
                                              problem_module_id)
    question_ids = [template + str(i + 2) + "_1"
                    for i in range(0, problem_module_size)]
...
def get_problem_module_size(problem_module):
    ...
    problem_module_size = len([element
                               for element in problem_module_as_xml.iter()
                               if element.tag in PROBLEM_TYPES])

    return  problem_module_size


def generate_answers_distribution_report(_xmodule_instance_args, _entry_id,
