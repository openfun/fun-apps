...
class CourseHandler(object):
    ...
    def make_thumbnail(self, size):
        if not self.memory_image_file:
            return None
        options = course_settings.THUMBNAIL_OPTIONS.copy()
        options['size'] = size
...
class Command(BaseCommand):
    ...
    def update_course_data(self):
        ...
        for mongo_course in self.courses:
            ...
            thumbnails_info = {}
            for size in course_settings.THUMBNAIL_POSSIBLE_SIZES:
                thumbnail = course_handler.make_thumbnail(size)
                if thumbnail:
                    size_key = '{}x{}'.format(*size)
                    thumbnails_info[size_key] = thumbnail.url
