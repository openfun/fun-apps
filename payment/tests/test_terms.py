...
class TermsAndConditionViewsTest(TestCase):
    ...
    @skipUnlessCms
    def test_acceptation_is_not_forced(self):
        """
        A CMS user should not be redirected to terms page until he accepted
        """
        self.client.login(username=self.user.username, password='test')
        # attempt to acces course list when still having to accept terms
        response = self.client.get(reverse("home"))
        # should freely access the page
        self.assertEqual(200, respnse.status_code)
