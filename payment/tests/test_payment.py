...
FAKE_COURSE_ID_STRING = "{}/{}/{}".format(
    FAKE_COURSE_ID.get("org"), FAKE_COURSE_ID.get("number"), FAKE_COURSE_ID.get("run")
)
...
class PayboxSystemViewsTest(AbstractPaymentTest):
    ...
    @patch('payment.utils.register_user_verified_cohort')
    @patch('payment.utils.get_order')
    def test_payment_notification_api(self, get_order_mock, register_user_verified_cohort_mock):
        get_order_mock.return_value = self.api_response
        data = {
        ...
        self.client.post(
            reverse('fun-payment-api:payment-notification'),
            json.dumps(data),
            content_type="application/json"
        )
        self.assertEqual(1, SoftwareSecurePhotoVerification.objects.filter(user=self.user).count())
        self.assertEqual(1, len(mail.outbox))
        self.assertTrue(mail.outbox[0].message().is_multipart())
        register_user_verified_cohort_mock.assert_called_once()
    ...
    def test_confirmation_email_language(self, get_order_mock):
        get_order_mock.return_value = self.api_response
        send_confirmation_email(self.user, self.api_response)
        self.assertEqual(1, len(mail.outbox))
...
class RegisterUserTest(ModuleStoreTestCase):
    ...
    def test_register_user_no_verified_cohort(self, add_user_to_cohort_mock):
        """
        Don't add user to any cohort when VERIFIED_COHORTS is not set.
        """
        register_user_verified_cohort(
            self.user, ecommerce_order_api_response("edX/toy/2012_Fall")
        )
        add_user_to_cohort_mock.assert_not_called()

    @patch("payment.utils.add_user_to_cohort")
    def test_register_user_add_to_cohort(self, add_user_to_cohort_mock):
        """
        Add user to default cohort when VERIFIED_COHORTS includes the catch-all pattern.
        """
        with override_settings(VERIFIED_COHORTS=[(r".*", "Certificat")]):
            register_user_verified_cohort(
                self.user, ecommerce_order_api_response("edX/toy/2012_Fall")
            )
        add_user_to_cohort_mock.assert_called_once_with(self.cohort, self.user.email)

    @patch("payment.utils.add_user_to_cohort")
    def test_register_user_verified_cohort_does_not_exist(
        self, add_user_to_cohort_mock
    ):
        """
        Don't add user to any cohort when VERIFIED_COHORTS points to a non-existing cohort.
        """
        with override_settings(VERIFIED_COHORTS=[(r".*", "Certificat qui n'existe pas")]):
            register_user_verified_cohort(
                self.user, ecommerce_order_api_response("edX/toy/2012_Fall")
            )
        add_user_to_cohort_mock.assert_not_called()
    ...
    @patch("payment.utils.add_user_to_cohort")
    def test_register_user_custom_course_cohort(self, add_user_to_cohort_mock):
        """
        Add user to the right course cohort (but not to the default cohort).
        """
        cohort2 = CourseUserGroup.objects.create(
            name="Certificat 2",
            course_id=self.course_key,
            group_type=CourseUserGroup.COHORT,
        )
        with override_settings(
            VERIFIED_COHORTS=[(self.course.key, "Certificat 2"), (r".*", "Certificat")]
        ):
            register_user_verified_cohort(
                self.user, ecommerce_order_api_response("edX/toy/2012_Fall")
            )
        add_user_to_cohort_mock.assert_called_once_with(cohort2, self.user.email)
